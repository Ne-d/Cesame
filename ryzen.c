...
static int detect_packages(void) {
  ...
  // Initialize all values of package_map to -1.
  for (i = 0; i < MAX_PACKAGES; i++)
    package_map[i] = -1;

  printf("\t");

  for (i = 0; i < MAX_CPUS; i++) { // For each CPU core
    sprintf(filename,
	...
	// Handle separating commas and new lines
    if (i % 8 == 7)
      printf("\n\t");
    else
      printf(", ");
...
static int rapl_msr_amd_core() {
  ...
  // Arrays, size of half the core count each (assumes SMT, probably about 2 logical cores per physical core)
  ...
  // Obtain and store fd for each core
  ...
  printf("Time_unit:%d, Energy_unit: %d, Power_unit: %d\n", time_unit,
         energy_unit, power_unit);

  // The MSR documentation said something about pow(0.5) but the link's broken so I can't check what it's about
  ...
  printf("Time_unit:%g, Energy_unit: %g, Power_unit: %g\n", time_unit_d,
         energy_unit_d, power_unit_d);

  int core_energy_raw;
  int package_raw;

  // Read per core energy values
  for (int i = 0; i < total_cores / 2; i++) { // For each core (or rather half of them cause we only read half)
  ...
  // Read per core energy values again, to get a delta
  for (int i = 0; i < total_cores / 2; i++) {
    core_energy_raw = read_msr(fd[i], AMD_MSR_CORE_ENERGY);
    package_raw = read_msr(fd[i], AMD_MSR_PACKAGE_ENERGY);
